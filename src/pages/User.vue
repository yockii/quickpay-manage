<template>
  <q-page class="q-pa-md q-gutter-md">
    <q-breadcrumbs>
      ...
      <q-breadcrumbs-el :label="$t('navigation.user')" icon="person" />
    </q-breadcrumbs>
    <!-- 筛选条件 -->
    <div class="items-start q-gutter-md row">
      <q-input v-model="condition.username" :label="$t('username')" />
      <!-- <q-space /> -->
  ...
  <q-dialog v-model="dialogAdd">
    <q-card style="width: 600px; max-width: 80vw" class="q-px-sm q-pb-md">
      ...
      <q-card-section>
        <div class="row">
          <q-input class="col-6" v-model="instance.username" :label="$t('username')" />
          <q-input
            class="col-6"
            type="password"
  ...
  <q-dialog v-model="dialogInfo">
    <q-card style="width: 600px; max-width: 80vw" class="q-px-sm q-pb-md">
      ...
      <q-card-section>
        <div class="row">
          <div class="col-6">
            <q-field :label="$t('username')" stack-label>
  ...
  <q-dialog v-model="dialogUpdate">
    <q-card style="width: 600px; max-width: 80vw" class="q-px-sm q-pb-md">
      ...
      <q-card-section>
        <div class="row">
          <div class="col-6">
            <q-input :label="$t('username')" v-model="instance.username" />
          </div>
          <div class="col-6">
            <q-input
              type="password"
    ...
    <q-card style="width: 600px; max-width: 80vw" class="q-px-sm q-pb-md">
      ...
      <q-card-section>
        <q-list bordered padding>
          <q-item tag="label" v-for="role in roles" :key="role.id">
            <q-item-section side top>
              <q-checkbox v-model="role.checked" />
            </q-item-section>
            <q-item-section>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function add() {
      if (instance.value.username === "" || instance.value.password === "") {
        return;
      }
      try {
        const resp = await user.add(instance.value);
        if (resp.code === 0) {
    ...
    async function update() {
      try {
        const resp = await user.updatePwd(instance.value);
        if (resp.code === 0) {
          if (resp.data) {
            $q.dialog({ message: $t("success") });
    ...
    async function openRoleDialog(u) {
      currentUser.value = u;
      try {
        const resp = await user.getRoleIds(u.id);
        if (resp.code === 0) {
          const ids = resp.data;
    ...
    async function dispatch() {
      ...
      try {
        const resp = await user.dispatchRoles({
